...
param([Parameter(Mandatory)][string]$PreviousSetupPath,[Parameter(Mandatory)][string]$UpdateSetupPath,
    [Parameter(Mandatory)][string]$SignedManifestPath,[Parameter(Mandatory)][string]$DataDirectory,[string]$DotnetPath='', [switch]$NativeParent)
...
if ([IO.File]::ReadAllText($game) -ne 'keep game data' -or [IO.File]::ReadAllText((Join-Path $installedRoot 'settings.json')) -ne $settings) { throw 'Update changed user data.' }
...
if (!$launched.success -or !$launched.detail.ui.text.Contains('mcmere Play ' + $handoff.expectedVersion)) { throw 'Updated application did not start with the expected version.' }
$result=[ordered]@{success=$true; signedManifestVerified=$true; realSetupHandoff=$true; parentExitWait=$true; nativeParent=$NativeParent.IsPresent; updatedVersion=$handoff.expectedVersion; appLaunched=$true; dataPreserved=$true; transport='synthetic release HTTP responses'; productionWindowsRegistrationTested=$false; realGameConnectionTested=$false}
