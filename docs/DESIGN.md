...
SyncEngineはZIPとoptions.txt内のパック選択を一つのjournalに記録し、commit前の中断では管理対象のファイルと選択だけを復旧する。音量などの他の設定は保持する。旧来の「options.txtはユーザー所有」に対する例外はresourcePacksとincompatibleResourcePacks内の管理ID、および自動有効化に必要なNeoForgeの基底ID `mod_resources`の補完だけであり、manifestでoptions.txt全体を配布することはできない。

options.txtのresourcePacksは低優先→高優先で、manifestの高優先順を反転して末尾へ追加する。NeoForge 21.1.250は未選択の必須mod_resourcesをTOP（この配列では末尾）へ挿入するため、自動有効化するパックがある場合は基底IDを明示する。未記録ならvanillaの直後（vanillaも未記録なら先頭）へ補い、既存の基底IDと個人パックの相対順序は保持する。これにより初回起動からMOD群より配布パックが高優先となり、既存環境も通常の準備・修復で是正できる。詳細とゲーム内の確認手順は[リソースパック優先順位の検証](RESOURCE_PACK_PRIORITY.md)を参照する。
...
3. `追加 / 置換 / 管理対象から削除 / 維持 / seed済み / 個人追加MOD`を計画する。配布管理対象のパスの大文字小文字衝突と同一modIdの重複はmanifestの検証で拒否する。
...
v0.1.1以降、配布管理対象外のJARは個人追加MODとして保持し、存在だけを理由に更新・修復・起動を保留しない。「MOD」画面に配布管理対象と分けて表示し、互換性は未確認と明記する。「確認済み」は配布管理対象の検証を指し、個人追加MODの動作を保証しない。管理対象と同じパスのファイルは引き続き指定版へ修復する。個人追加MODのmodId・依存関係・ローダー互換性の自動判定は行わず、配布MODとの別版重複を避けるよう案内する。利用者が「追加MODの退避を確認」から明示的に選んだ場合だけ一括退避する。旧版で退避済みのJARは自動復元しない。
